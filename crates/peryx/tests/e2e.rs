//! End-to-end regression tests: real pip/uv/twine driven against a spawned peryx binary, proving
//! downstream clients work through peryx for real.
//!
...
/// wheel and its PEP 658 `.metadata` sibling with the sha256s peryx will verify against.
...
impl Peryx {
    /// Spawn peryx proxying the given upstream (a fixture, or pypi.org) and wait until it answers.
    ...
    /// test's peryx can grab the port in the window, our child dies at bind, and the health check
...
/// listener during the port-race window) reads as not-ready, and the body must identify peryx.
...
/// Stand up a hermetic fixture upstream and a peryx proxying it. Both live until the tuple drops.
...
/// A raw GET that asks for HTML (peryx negotiates on Accept), returning the body.
...
fn e2e_pip_installs_and_resolves_dependencies() {
    ...
    let venv = uv_venv();
    pip_install(&venv, &peryx, "peryxa");
    assert_importable(&venv, "peryxa");
    assert_importable(&venv, "peryxb"); // transitive dependency resolved through peryx
}
...
fn e2e_pip_uses_pep658_metadata_fast_path() {
    ...
    assert!(
        ...
        "pip did not fetch a .metadata sibling through peryx"
    );
}
...
fn e2e_uv_uses_pep658_metadata_fast_path() {
    ...
    assert!(
        peryx.metadata_requests() >= 1,
        "uv did not fetch a .metadata sibling through peryx"
    );
}
...
fn e2e_pip_respects_policy_blocked_dependency() {
    ...
    let venv = uv_venv();
    pip_install_fails(&venv, &peryx, "peryxa");
}
...
fn e2e_uv_respects_policy_blocked_dependency() {
    let (_upstream, peryx) = hermetic_with_overlay_policy("block_projects = [\"peryxb\"]\n");
    let venv = uv_venv();
    uv_install_fails(&venv, &peryx, "peryxa");
}
...
fn e2e_yank_and_delete_round_trip() {
    ...
    assert_eq!(http_verb(peryx.port, "PUT", "/root/pypi/peryxremove/1.0/yank"), 200);
    ...
    assert_eq!(http_verb(peryx.port, "DELETE", "/root/pypi/peryxremove/"), 200);
...
fn e2e_live_pip_installs_from_pypi_via_pep658() {
    let peryx = live();
    let venv = uv_venv();
    pip_install(&venv, &peryx, "certifi");
...
fn e2e_upstream_yank_hide_restore_round_trip() {
    ...
    assert_eq!(http_verb(peryx.port, "PUT", "/root/pypi/peryxa/1.0/yank"), 200);
    ...
    assert_eq!(http_verb(peryx.port, "DELETE", "/root/pypi/peryxa/1.0/yank"), 200);

    // Hide it outright, confirm it vanished, then restore it.
    assert_eq!(http_verb(peryx.port, "DELETE", "/root/pypi/peryxa/"), 200);
    ...
    assert_eq!(http_verb(peryx.port, "PUT", "/root/pypi/peryxa/restore"), 200);
