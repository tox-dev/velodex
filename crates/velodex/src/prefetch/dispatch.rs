...
fn mirror_driver(state: &Arc<AppState>, name: &str) -> &'static dyn IndexMirror {
    let is_oci = state
        .indexes
        .iter()
        .any(|index| (index.name == name || index.route == name) && index.ecosystem == velodex_format::Ecosystem::Oci);
...
impl IndexMirror for OciMirror {
    ...
    async fn sync(
        ...
        oci_mirror(
            ...
            &images,
            velodex_ecosystem_oci::MirrorMode::Sync,
            out,
        )
        .await
    }
    async fn verify(
        ...
        oci_mirror(
            ...
            oci_lookup(state, &options.index),
            &images,
            velodex_ecosystem_oci::MirrorMode::Verify,
            out,
        )
        .await
    }
