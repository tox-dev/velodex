...
rule, and it is that rule that is a security control before it is a convenience.
...
- **role** is what the index does. There are three:
  - **cached** — a read-through cache of one upstream index. "Upstream" means the index velodex fetches from, e.g.
  ...
  - **hosted** — an authoritative store you upload to. Nothing upstream; the packages live here because you published
    them. (This was called a "local" index.)
  - **virtual** — an ordered aggregation of other indexes served under one URL. Its member list is called `layers`.
    (This was called an "overlay".)
- **ecosystem** is which packaging format the index speaks: **pypi** today, with OCI and npm planned. It fixes the wire
  protocol (for PyPI, the [PEP 503/691](@/explanation/standards.md) Simple API) and the artifact shapes (wheels and
  sdists). See [ecosystems](@/ecosystems/_index.md).

The axes are orthogonal at creation but coupled at aggregation: a virtual index may only combine members of the **same
ecosystem**. Everything below describes the roles for the PyPI ecosystem, the only one shipping today; the same shapes
apply unchanged as ecosystems are added.
...
Out of the box velodex runs a pypi.org cached index plus an empty hosted index, combined by a virtual index at
`root/pypi`. One URL therefore serves the whole public index, and the day you need to host a private package you add a
token; nothing about the client setup changes.

## In practice

- Build the topology: [compose virtual indexes](@/guides/compose-overlays.md),
  [proxy a private upstream](@/guides/private-mirror.md)
- Publish into it: [publish](@/guides/publish.md); undo things: [yank and delete](@/guides/remove.md)
- The vocabulary in one place: [glossary](@/reference/glossary.md)
