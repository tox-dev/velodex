...
- **A streaming cold path.** pypicloud buffers a missed wheel fully into a `TemporaryFile`, writes it to storage and a
  ...
  velodex [streams the bytes to the client and into the store at once](@/core/architecture.md).
...
| pypicloud                                | velodex                                                                   |
| ---------------------------------------- | ------------------------------------------------------------------------- |
| `ppc-make-config` + `pserve config.ini`  | a [TOML file](@/core/configuration.md) + `velodex serve`                  |
