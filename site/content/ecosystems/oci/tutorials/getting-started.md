...
description = "Serve container images through velodex: cache Docker Hub, pull an image, push one of your own, and verify it."
weight = 1
+++

In this tutorial you configure velodex as a container registry, start it, pull an image through a cached proxy of Docker
...
(`latest`, `1.0`) points at a manifest. velodex serves all of this over the container `/v2/` protocol that `docker`,
...
velodex binary. Pick whichever install channel fits; [installation](@/core/installation.md) lists them all:
...
```shell
velodex serve --config velodex.toml   # ./target/release/velodex serve when built from source
```
...
so on the same host the commands below work as written. Reaching velodex over the network (or from Docker Desktop, whose
...
Pushing needs the hosted index's `upload_token`. velodex accepts any username; the token is the Basic-auth password. Log
...
velodex also serves a web interface on the same port. Open [http://127.0.0.1:4433/](http://127.0.0.1:4433/) for a live
