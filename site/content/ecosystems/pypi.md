...
`.tar.gz`) is the source a wheel is built from. Both are **artifacts** — the actual files an installer fetches. This is
the one ecosystem velodex ships today.

## The roles for PyPI

The three [index roles](@/explanation/indexes.md) map onto PyPI like this:
...
For the full standards map, see [standards](@/explanation/standards.md).
...
Publishing needs a [hosted layer with an `upload_token`](@/guides/publish.md). velodex accepts any username; the token
is the password, matching pypi.org's `__token__` convention.
...
OCI (container images) and npm (JavaScript) are **planned**: the same cached/hosted/virtual roles apply, driven by each
format's own protocol. The [capability matrix](@/reference/capabilities.md) tracks where each stands.

## In practice

- Front an index that is not pypi.org: [front another index](@/tutorials/front-another-index.md)
- Add credentials for a private upstream: [proxy a private upstream](@/guides/private-mirror.md)
- Publish your own packages: [publish](@/guides/publish.md)
