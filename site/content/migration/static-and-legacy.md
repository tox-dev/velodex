...
[air-gap guide](@/guides/air-gapped.md) with a cache instead of a script: the carried data directory keeps hashes,
metadata, and the JSON API.
...
`Cache-Control`-driven refresh with a background sweep, private hosting, and [usage counters](@/guides/monitor.md).
