...
populated on first use at [no install-time penalty](@/explanation/performance.md) or ahead of time with
`velodex mirror sync`, serves it itself (no nginx layer, no name-normalization rewrite rules), stays as fresh as
...
- Read-through mode has a warm-up: nothing is present until requested. For an air gap, run `velodex mirror sync` on a
  connected network and [carry the data directory across](@/guides/air-gapped.md).
- `--mode all` walks the upstream root Simple index, but velodex does not implement PyPI's serial mirror protocol. If an
  auditor requires that protocol, stay with bandersnatch.
- Policy applies when velodex serves, mirrors, caches, or accepts an upload. Prefetch filters decide what
  `velodex mirror sync` brings in ahead of demand.
